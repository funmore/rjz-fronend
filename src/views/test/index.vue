<template>
  <div class="components-container">
    ...
    <el-table  ref="dataTable" :data="tableData" :span-method="objectSpanMethod" @cell-dblclick="cellDbClick" @row-contextmenu="rowContextmenu" border  stripe>
        ...
        <el-table-column v-for="index in treeDataLength" :key='index'  align="center" :label="'对象序号'+index" width="80">
...
<!--                 <el-button type="warning" icon="el-icon-tickets" circle @click="appendToTree(scope.row[index-1].id)"></el-button>
      ...
      <el-table-column v-for="index in proceData.length" :key='index'  align="center" :label="proceData[index-1]" width="80">
        <template slot-scope="scope">
   ...
   watch: {
     treeDataLength: function(newVa, oldVa) {
       if (newVa > this.treeDataLengthOld) {
         ...
         for (let i = this.treeDataLengthOld; i > newVa; i--) {
   ...
   methods: {
     ...
     deleteFromTree(nodeid) {
       ...
       if (index !== -1) {
         ...
         for (var i = 0; nodeParent == null && i < treeData.length; i++) {
           nodeParent = this.getParentFromOneTree(treeData[i], nodeid)
         }
     ...
     getTreeTableData() {
       ...
       this.treeTableData.forEach(value => {
         if (value.length < this.treeDataLength) { var len = this.treeDataLength - value.length }
         for (var i = 0; i < len; i++) {
     ...
     getTreeData(treeTableData, retTree, columnIndex, startRow, endRow) {
       ...
       retTree = arguments[1] ? arguments[1] : []// 设置参数retTree的默认值为[]
       columnIndex = arguments[2] ? arguments[2] : 0// 设置参数columnIndex的默认值为[]
       startRow = arguments[3] ? arguments[3] : 0// 设置参数startRow的默认值为[]
       endRow = arguments[4] ? arguments[4] : treeTableData.length// 设置参数endRow的默认值为[]
